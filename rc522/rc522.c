...
unsigned char  read_reg (unsigned char addr)
{
    ...
    val[0] = addr;
    val[1] = 0;
    status = spi_read(fd, val);
    if(status < 0)
    {
        printf("can't read byte form device\n");
    }
    return status;
} /* ----- End of read_reg()  ----- */
...
void calulatate_crc (unsigned char *indata, unsigned char len, unsigned char *outdata)
{
    ...
    for(int n=0; n<len; n++){
    	write_reg(FIFO_DATA_REG, *(indata+n));
    } 
    ...
    do
    {
        reg_val = read_reg(DIV_IRQ_REG);
        get_cur_time(&cur_usec);
    }
    while (!(last_usec + 25000 < cur_usec) && !(reg_val & 0x04));

    outdata[0] = read_reg(CRC_RESULT_REG_L);
    outdata[1] = read_reg(CRC_RESULT_REG_M);
        ...
        unsigned int  *outlenbit)
{
    ...
    do
    {
        com_irq_reg = read_reg(COM_IRQ_REG);
        get_cur_time(&cur_usec);
    }
    ...
    if(!(last_usec + 25000 < cur_usec))
    {
        ...
        if(!(reg_val & 0x1B))
        {
            ...
            if(command == PCD_TRANSCEIVE)
            {
                    reg_val = read_reg(FIFO_LEVEL_REG);
                    last_bits = read_reg(CONTROL_REG);
                    ...
                    for(i = 0; i < reg_val; i++)
                    {
                        outdata[i] = read_reg(FIFO_DATA_REG);
                    }
            }
        }
        else
...
void rfid_antenna_on (void)
{
    ...
    reg_val = read_reg(TX_CONTROL_REG);
    if(!(reg_val & 0x03))
    {
        set_bit_mask(TX_CONTROL_REG, 0x03);
    }
